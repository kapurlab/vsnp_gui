...
- **T-46 Single-end + junk-fastq handling.** Surfaced during the LSDV India batch 2 dispatch (2026-05-18) — 5 of the 78 SRR/DRR runs in the new batch were single-end submissions (no `_1`/`_2` paired fastqs). The T-07 provenance dispatch at `provenance_writer.py:1199` strictly requires both R1 and R2 patterns; a single single-end sample aborts the *entire* batch with `Provenance dispatch failed (step1): no R1 fastq found for sample SRRxxx`. Two of those five were also <50 KB (43 KB and 47 KB) — clearly junk submission errors that shouldn't reach Step 1 in the first place. **Phase 1 done (commit pending):** new `_step1_dispatch_plan()` in `backend/app/main.py` auto-filters at dispatch time. Skips single-end-only (no R1 match), R1-without-R2 (incomplete download), and any paired set where either fastq is <1 MB (junk floor). Returns the skipped list in the `/step1/run` response so the GUI shows an alert listing which samples were excluded and why. No manual ssh+rename needed per batch any more. **Phase 2 deferred:** real single-end Illumina support. Would require a vsnp3 patch (or wrapper-level `--single-end` flag plumbed through to bwa/freebayes), plus updating `_build_step1_inputs` to allow R2-absent for single-end mode. Add to `deploy/vsnp3-patches/v3.16-kapurlab.patch` alongside T-43's seqkit fix.
